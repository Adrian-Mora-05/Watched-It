...
export default function MovieReviewScreen() {
  ...
  const { id, movie_id, name } = useLocalSearchParams<{ id: string; movie_id: string; name: string }>()
  ...
  const fetchReviews = async (reset = false) => {
    ...
    try {
      ...
      const data = await getMovieReviews(Number(movie_id), id_usuario!, currentSkip, limit)
  ...
  const handleLike = async (review: Review) => {
    ...
    try {
      if (isLiked) {
        await removeLike(review.id, 'pelicula', token!)
      } else {
        await addLike(review.id, 'pelicula', token!)
      }
    } catch (e) {
      setReviews(prev => prev.map(r =>
        r.id === review.id
